...
## Non-interactive server workflow

The added scripts keep model weights, NetCDF data, logs, and predictions out of
the source tree. Their default runtime root is
`/root/group_data/SDU-Test/weather`.

Run the environment and repository audit before changing any package:

```bash
cd ~/weather
python environment_check.py
cat /root/group_data/SDU-Test/weather/logs/environment.log
```

In particular, inspect the recorded Python, `jax`, `jaxlib`, Haiku, backend,
and `jax.devices()` values. Do not replace an accelerator-specific JAX build
just to match a generic version. `requirements.txt` intentionally omits JAX
and jaxlib; install only dependencies proven missing after this audit.

List the public bucket contents, then download the minimal real
GraphCast_small checkpoint, compatible ERA5 1-degree/13-level/1-step example,
...
`run.sh` repeats the environment check, verifies every required asset, and
writes all runtime artifacts below the external data root. The output includes
`predictions.nc`, `metrics.json`, a small PNG, and timestamped logs. The metrics
record first JIT-plus-inference time, second warm inference time, dimensions,
per-variable MAE/RMSE where targets exist, and NaN/Inf counts.

GraphCast uses JAX/Haiku. These scripts never call `flag_gems.enable()` and do
not claim FlagGems acceleration. A run is labelled `CPU baseline` whenever
`jax.default_backend()` is `cpu`; accelerator support is reported only from
the actual JAX backend and devices.

...
def validate_manifest(path):
    ...
    known_dataset_ids = {
        item.get("id")
        for item in required_datasets + relations.get("optional_datasets", [])
...
def validate_relations(path):
    relations_doc = load_yaml(path)
    if relations_doc.get("model_id") != MODEL_ID:
        fail("onescience_relations.yaml 的 model_id 不正确")
    dataset_refs = relations_doc.get("relations", {}).get("required_datasets", [])
    if not any(item.get("id") == DATASET_ID for item in dataset_refs):
        fail("onescience_relations.yaml 未声明 OneScience/ERA5")
    ok("onescience_relations.yaml 与模型/数据集关系一致")


def main():
    parser = argparse.ArgumentParser(description="校验 FuXi ModelScope 标准模型包")
    ...
    relations_path = os.path.join(args.root, "onescience_relations.yaml")
    for path in (readme_path, manifest_path, compat_manifest_path, relations_path):
    ...
    validate_relations(relations_path)

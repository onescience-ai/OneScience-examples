...
class CuGraphCSC:
    ...
    def __init__(
        ...
        raise NotImplementedError(
            "Distributed graph partitioning is not included in this standalone "
            "GraphCast package. Use partition_size=1 or run standard DGL graphs."
        )
    ...
    def from_dgl(
        ...
        if partition_by_bbox and partition_size > 1:
            raise NotImplementedError(
                "Latitude/longitude graph partitioning is not included in this "
                "standalone GraphCast package. Use partition_size=1."
            )
    ...
    def to_dgl_graph(self) -> DGLGraph:  # pragma: no cover
        ...
        graph in graphcast_src which assumes a simple CSC structure, but

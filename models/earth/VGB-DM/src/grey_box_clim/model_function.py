...
class Climate_encoder_free_uncertain(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.layers = [5, 3, 2]
        self.hidden = [96, 64, 2 * out_types]  # [128,64,2*out_types]
        ...
        if use_err:
            self.noise_net = Climate_ResNet_2D(
                err_in, [3, 2, 2], [96, 64, 2 * out_types]
            )
        if use_pos:
    ...
    def pde(self, t, vs):
        ...
        if self.pos:
            comb_rep = torch.cat(
                [t_emb / 24, day_emb, seas_emb, nabla_u, v, ds, self.pos_feat],
                dim=1,
            )
        else:
    ...
    def noise_net_contrib(self, t, pos_enc, s_final, noise_net, H, W):
        ...
        comb_rep = torch.cat(
            [
                t_cyc_emb,
                s_final.flatten(start_dim=0, end_dim=1),
                pos_enc,
                pos_time_ft,
            ],
            dim=1,
    ...
    def forward(self, T, data, atol=0.1, rtol=0.1):
        ...
        if self.err:
            ...
            s_final = s_final[0 : len(s_final) : 6]
            mean = s_final
            std = torch.zeros_like(mean) + 1e-3

        return mean, std, s_final

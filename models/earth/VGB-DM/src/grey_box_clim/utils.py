...
def get_train_test_data_without_scales_batched(
    data_path, train_time_scale, val_time_scale, test_time_scale, lev, start_y
):
    ...
    data_global = data.sel(time=slice(str(start_y), "2018")).load()
    ...
    train_times = [i for i in range(start_y, 2016)]
...
def get_train_test_data_batched_regional(
    ...
    data_global = data.sel(time=slice("2010", "2018")).load()
    ...
    time_vals = data_test_final.time.values
    train_times = [i for i in range(2010, 2016)]
    test_times = [2017, 2018]
    val_times = [2016]
...
def evaluation_rmsd_mm(Pred, Truth, lat, lon, max_vals, min_vals, H, W, levels):
    ...
    for idx, lev in enumerate(levels):
        ...
        RMSE_lat_lon.append(lat_lon_rmse[lev].values)
        RMSE_final.append(rmse[lev].values.tolist())
...
def evaluation_rmsd_mm_region(Pred, Truth, lat, lon, max_vals, min_vals, H, W, levels):
    ...
    for idx, lev in enumerate(levels):
        ...
        das_pred.append(
            xr.DataArray(
                pred.reshape(1, H, W - 1),
                dims=["time", "lat", "lon"],
                coords={"time": [0], "lat": lat[1:], "lon": lon[1:]},
                name=lev,
            )
        )
        Pred_xr = xr.merge(das_pred)
...
def get_gauss_kernel(shape, lat, lon):
    cwd = os.getcwd()
    rows, columns = shape
    # If CUDA is unavailable, return a random matrix with same shape
    if not torch.cuda.is_available():
        return torch.rand(shape[0] * shape[1], shape[0] * shape[1])
...
def get_gauss_kernel_region(shape, lat, lon, region):
    ...
    kernel_inv = torch.linalg.inv(kernel).numpy()
    np.save("./experiments/dataset" + "/kernel_" + str(region) + ".npy", kernel_inv)
...
def optimize_vel(num, data, delta_u, vel_model, kernel, H, W, steps=200):
    ...
    for step in range(steps):
        sys.stdout.write(f"\rFitting Velocity: Epoch {step}: ")
        sys.stdout.flush()
        optimizer.zero_grad()
        ...
        vel_loss = nn.MSELoss()(delta_u, out.squeeze(dim=1)) + 0.0000001 * (
            final_x + final_y
        )
...
def fit_velocity(
    ...
    for idx_steps, time_steps, batch in zip(time_idx, time_loader, data_loader):
        ...
        delta_u = get_delta_u(past_sample, past_time).to(device)
        steps = 200 if torch.cuda.is_available() else 1
        v_x, v_y, loss_terms, out = optimize_vel(
            num_years, data, delta_u, vel_model, kernel, H, W, steps=steps
        )
        # v_x, v_y, loss_terms, out = optimize_vel(num_years, data, delta_u, vel_model, kernel, H, W, steps=1)
    ...
    if os.path.exists("./experiments/dataset" + "/" + types + "_vel.npy"):
        os.remove("./experiments/dataset" + "/" + types + "_vel.npy")

    # np.save(str(cwd) +"/" + types + "_vel.npy",Final_v.detach().numpy())
    res = Final_v.detach().cpu().numpy()
    np.save(str(cache_dir) + "/" + types + "_vel.npy", res)
    return torch.from_numpy(res)


def load_velocity(types, cache_dir):
    vel = []
    for file in types:
        vel.append(np.load(str(cache_dir) + "/" + file + "_vel.npy"))

    return (torch.from_numpy(v) for v in vel)


def load_velocity_region(types, cache_dir):
    vel = []
    for file in types:
        vel.append(np.load(str(cache_dir) + "/region" + file + "_vel.npy"))
...
def get_train_test_data_without_scales_batched_monthly(
    ...
    data_val = data.sel(time=val_time_scale).load()
    data_test = data.sel(time=test_time_scale).load()
    data_global = data.sel(time=slice(start_y, "2018")).load()
    ...
    time_vals = data_test_final.time.values
    train_times = [i for i in range(int(start_y), 2016)]
    test_times = [2017, 2018]
    val_times = [2016]

...
from onescience.models.pinnsformer import QRes2D
...
model = QRes2D(in_dim=3, hidden_dim=128, out_dim=2, num_layer=4).to(device)
...
if not os.path.exists('./model'):
    os.makedirs('./model')
torch.save(model.state_dict(), './model/ns_qres.pt')
...
plt.tight_layout()
plt.savefig('./result/ns_qres.png')
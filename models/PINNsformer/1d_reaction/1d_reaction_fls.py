...
from onescience.models.pinnsformer import FLS1D
seed = 0
np.random.seed(seed)
random.seed(seed)
...
# Train PINNs 
model = FLS1D(in_dim=2, hidden_dim=128, out_dim=1, num_layer=4).to(device)
...
if not os.path.exists('./model'):
    os.makedirs('./model')
torch.save(model.state_dict(), './model/1dreaction_fls.pt')
...
plt.tight_layout()
plt.savefig('./result/1dreaction_fls.png')

...
SETUP_BLOCK
    echo "set +u
if [ -n \"\${ONESCIENCE_MATCHEM_ENV:-}\" ] && [ -f \"\$ONESCIENCE_MATCHEM_ENV\" ]; then
  source \"\$ONESCIENCE_MATCHEM_ENV\"
elif [ -f \"\${ONESCIENCE_ROOT:-}/examples/matchem/matchem_env.sh\" ]; then
  source \"\$ONESCIENCE_ROOT/examples/matchem/matchem_env.sh\"
else
  echo \"[WARN] 未找到 matchem_env.sh，假设当前环境已安装 OneScience matchem 依赖\"
fi
set -u" >> "$SLURM_SCRIPT"

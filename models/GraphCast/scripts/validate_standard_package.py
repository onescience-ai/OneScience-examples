...
def validate_readme(path):
    ...
    if f"modelscope download --model {MODEL_ID}" not in text:
        fail("README 模型下载命令与上传目标 ID 不一致")
    if f"modelscope download --dataset {DATASET_ID}" not in text:
        fail("README 数据集下载命令与数据集 ID 不一致")
...
def validate_manifest(path):
    ...
    bad_model_ids = re.findall(r"OneScience/GraphCast(?=[\"'\\s])", yaml_text)
    if bad_model_ids:
...
def main():
    parser = argparse.ArgumentParser(description="校验 GraphCast ModelScope 标准模型包")
    ...
    standardized_root = os.path.dirname(os.path.dirname(abs_root))
    dataset_package = os.path.join(standardized_root, "dataset", "era5")
    if os.path.exists(dataset_package):
        fail(f"按任务要求跳过数据集整理，但发现数据集标准仓库目录: {dataset_package}")
    ok("按要求未生成数据集标准仓库")

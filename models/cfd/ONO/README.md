...
ONO（Orthogonal Neural Operator）通过注意力机制提取网格点之间的全局关系，并在物理场特征传播过程中引入正交投影，缓解深层神经算子中的特征退化和过平滑问题。
...
本仓库是 OneScience 整理的 ONO 最小可运行独立模型仓库，面向 ModelScope 下载、OneCode 自动化运行和本地快速验证场景。

当前支持能力：

- 使用统一 YAML 配置训练和推理二维 ONO
- 使用 Nyström、线性或高效自注意力建模网格全局关系
- 通过协方差白化和正交投影更新物理场特征
...
| `model/ono.py` | 独立 ONO 模型 | 不依赖 `onescience.modules` |
...
默认检查点保存至 `weight/ono_navier_stokes.pt`。训练脚本完全由 `conf/config.yaml` 驱动，不接收命令行配置参数。
...
脚本默认读取 `weight/ono_navier_stokes.pt`，并在 `result/` 下生成：
...
- `model`：隐藏维度、正交块、注意力、MLP 和投影空间配置
...
- `num_layers`：Orthogonal Neural Block 数量
- `num_heads`：注意力头数，`hidden_dim` 必须能被其整除
- `attn_type`：支持 `nystrom`、`linear` 和 `selfAttention`
- `psi_dim`：用于协方差白化和正交更新的投影空间维度
- `mlp_ratio`：每个正交块中 MLP 隐层扩展倍率
...
- Transolver PDE-Solving Standard Benchmark: https://github.com/thuml/Transolver/tree/main/PDE-Solving-StandardBenchmark

...
class DiffusionReactionEquation(object):
    ...
    def pe_boundary(self, x_l, x_r, n=100):
        ...
        return xt_bd_l.to(self.device), xt_bd_r.to(self.device)
    ...
    def generate_test(self, dataset, N=10000):
        with h5py.File(dataset, 'r') as f:
            ...
            t = f['t-coordinate'][:-1].reshape(-1, 1)
...
def loss(eq, model_u, model_v, xt_int, xt_bd_l, xt_bd_r, xt_init, xt_end, ue_init, beta):
    ...
    int_l2 = st_s * torch.mean(u ** 2 * (w * v))
    int_r3 = st_s * torch.mean(u * (w * v))
    int_l3 = 0.5 * st_s * torch.mean(dux * (wx * v + w * dvx))
...
def train_pipeline():
    ...
    equation = 'DR1'
    dataset = '../../data/ReacDiff_Nu0.5_Rho1.0.hdf5'
    # define equation
    Eq = DiffusionReactionEquation(DIMENSION, DEVICE)
    # define model

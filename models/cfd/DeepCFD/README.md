...
DeepCFD: Efficient Steady-State Laminar Flow Approximation (arXiv:2004.08826)

论文复现：本模型由 OneScience 基于论文复现，使用 DCU 集群训练，产出可运行代码、配置与 checkpoint。

# 模型描述

模型架构：U-Net with 3 independent decoders (UNet-3)
...
```bash
conda create -n deepcfd python=3.11 -y
conda activate deepcfd
...
```bash
python scripts/train.py --config conf/deepcfd.yaml
```

### 训练权重

- `deepcfd_tier1_model.pt`

### 推理

```bash
python scripts/evaluate.py --config conf/deepcfd.yaml --run-dir <run_dir>
```
...
论文引用：DeepCFD: Efficient Steady-State Laminar Flow Approximation (arXiv:2004.08826)

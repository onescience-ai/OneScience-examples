...
class GUNet(nn.Module):
    def __init__(self, hparams, encoder, decoder):
        ...
        if self.layer == 'SAGE':
            self.down_layers.append(nng.SAGEConv(self.dim_enc, self.size_hidden_layers))
            bn_in = self.size_hidden_layers
        elif self.layer == 'GAT':
            self.down_layers.append(
                nng.GATConv(self.dim_enc, self.size_hidden_layers, heads=self.head,
        ...
        for n in range(1, self.L):
            if self.pool_type != 'random':
                self.pool.append(
                    nng.TopKPooling(self.size_hidden_layers, ratio=self.pool_ratio[n - 1],
                                    nonlinearity=torch.sigmoid)
                )
            if self.layer == 'SAGE':
                self.down_layers.append(
                    nng.SAGEConv(self.size_hidden_layers, 2 * self.size_hidden_layers)
                )
                self.size_hidden_layers = 2 * self.size_hidden_layers
                bn_in = self.size_hidden_layers
            elif self.layer == 'GAT':
                self.down_layers.append(
                    nng.GATConv(self.head * self.size_hidden_layers, self.size_hidden_layers,
        ...
        if self.layer == 'SAGE':
            self.up_layers.append(
                nng.SAGEConv(3 * self.size_hidden_layers_init, self.dim_enc)
            )
            self.size_hidden_layers_init = 2 * self.size_hidden_layers_init
        elif self.layer == 'GAT':
            self.up_layers.append(
                nng.GATConv(2 * self.head * self.size_hidden_layers, self.dim_enc,
                            heads=2, add_self_loops=False, concat=False)
            )
        ...
        for n in range(1, self.L - 1):
            if self.layer == 'SAGE':
                self.up_layers.append(
                    nng.SAGEConv(3 * self.size_hidden_layers_init, self.size_hidden_layers_init)
                )
                bn_in = self.size_hidden_layers_init
                self.size_hidden_layers_init = 2 * self.size_hidden_layers_init
            elif self.layer == 'GAT':
                self.up_layers.append(
                    nng.GATConv(2 * self.head * self.size_hidden_layers, self.size_hidden_layers,
                                heads=2, add_self_loops=False, concat=True)
                )
            if self.bn_bool:
                self.bn.append(nng.BatchNorm(in_channels=bn_in, track_running_stats=False))

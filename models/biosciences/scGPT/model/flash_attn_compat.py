...
_flash_attn_enabled = os.environ.get(
    "ONESCIENCE_SCGPT_ENABLE_FLASH_ATTN", ""
).lower() in {"1", "true", "yes"}
_flash_attn_can_initialize = _flash_attn_enabled and torch.cuda.is_available()
...
if not _flash_attn_can_initialize:
    reason = (
        "no CUDA/DCU device is available"
        if _flash_attn_enabled
        else "set ONESCIENCE_SCGPT_ENABLE_FLASH_ATTN=1 to enable probing"
    )
    flash_attn_error = f"flash-attn probe skipped: {reason}"
else:
    try:
...
def get_flash_attn_parameter_rename_rules(
    ...
    **Issue Background:**
    ...
    Example:
        ...
        >>> # rules map direct attention keys to wrapped attention keys.
    """

...
output_dir="${PROJECT_ROOT}/outputs/example_4_non_fixed_outputs"
if [ ! -d "$output_dir" ]
then
    mkdir -p "$output_dir"
fi
...
python "${PROJECT_ROOT}/scripts/helper_scripts/make_fixed_positions_dict.py" --input_path="$path_for_parsed_chains" --output_path="$path_for_fixed_positions" --chain_list "$chains_to_design" --position_list "$design_only_positions" --specify_non_fixed

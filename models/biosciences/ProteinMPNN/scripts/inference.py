...
def _resolve_model_folder_path(args):
    if args.path_to_model_weights:
        return os.path.abspath(os.path.normpath(args.path_to_model_weights))
    if args.ca_only and args.use_soluble_model:
        raise ValueError("CA-SolubleMPNN is not available yet")
    if args.ca_only:
        variant = "ca_model_weights"
    elif args.use_soluble_model:
        variant = "soluble_model_weights"
    else:
        variant = "vanilla_model_weights"
    return os.path.join(_PROJECT_ROOT, "weight", variant)
...
def main(args):
    ...
    try:
        model_folder_path = _resolve_model_folder_path(args)

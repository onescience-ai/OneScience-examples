...
def run_esmfold(
    ...
    esmfold_model_path: Optional[str] = None,
) -> List[str]:
    """
    ...
    This version supports loading ESMFold from a local directory through an
    environment variable, so it will not download facebook/esmfold_v1 again.
    ...
    Args:
        ...
        cache_dir: Cache directory for model weights. Kept for compatibility, but
            local ESMFold loading is preferred when ESMFOLD_MODEL_PATH is set.
        keep_outputs: Whether to keep output directories
        esmfold_model_path: Optional explicit local path to ESMFold. If None,
            use environment variable ESMFOLD_MODEL_PATH, then fallback to the
            cluster shared model path.

    Returns:
        List of paths (list of str) to PDB files
    """
    default_esmfold_model_path = os.path.join(
        os.environ.get("ONESCIENCE_MODELS_DIR", ""), "esmfold_v1_safe"
    )

    local_esmfold_path = (
        esmfold_model_path
        or os.environ.get("ESMFOLD_MODEL_PATH")
        or default_esmfold_model_path
    )
    ...
    if not os.path.isdir(local_esmfold_path):
        raise FileNotFoundError(
            f"Local ESMFold model directory not found: {local_esmfold_path}\n"
            "Please set a valid path, for example:\n"
            "export ESMFOLD_MODEL_PATH=$ONESCIENCE_MODELS_DIR/esmfold_v1_safe"
        )

    missing_files = [
        f for f in required_files
        if not os.path.exists(os.path.join(local_esmfold_path, f))
    ]
    if missing_files:
        raise FileNotFoundError(
            f"Local ESMFold model directory is incomplete: {local_esmfold_path}\n"
            f"Missing files: {missing_files}"
        )

    logger.info(f"Loading ESMFold from local path: {local_esmfold_path}")

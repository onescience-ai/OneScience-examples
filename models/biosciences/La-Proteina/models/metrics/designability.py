...
from .folding_models import run_esmfold
from models.utils.align_utils import kabsch_align_ind
from models.utils.coors_utils import (
    get_atom37_bb3_mask,
    get_atom37_bb3o_mask,
    get_atom37_ca_mask,
)
from models.utils.pdb_utils import load_pdb
...
def run_proteinmpnn(
    ...
    proteinmpnn_dir = Path(
        os.environ.get(
            "PROTEINMPNN_DIR",
            repo_root / "examples" / "biosciences" / "ProteinMPNN",
        )
    )
    proteinmpnn_script = proteinmpnn_dir / "protein_mpnn_run.py"
    if not proteinmpnn_script.exists():
        raise RuntimeError(
            "ProteinMPNN script not found. Set PROTEINMPNN_DIR to the directory "
            f"containing protein_mpnn_run.py. Tried: {proteinmpnn_script}"
        )

    proteinmpnn_weights_dir = os.environ.get("PROTEINMPNN_WEIGHTS_DIR")
    if proteinmpnn_weights_dir is None and os.environ.get("ONESCIENCE_MODELS_DIR"):
        weight_subdir = "ca_model_weights" if ca_only else "vanilla_model_weights"
        proteinmpnn_weights_dir = str(
            Path(os.environ["ONESCIENCE_MODELS_DIR"]) / "ProteinMPNN" / weight_subdir
        )

    command = shlex.split(python_exec) + [
        str(proteinmpnn_script),
        "--pdb_path",
        pdb_file_path,
        "--pdb_path_chains",
        " ".join(pdb_path_chains),
        "--out_folder",
        out_dir_root,
        "--num_seq_per_target",
    ...
    if proteinmpnn_weights_dir:
        command.extend(["--path_to_model_weights", proteinmpnn_weights_dir])
    ...
    if fix_pos:
        ...
        command.extend(["--fixed_positions_jsonl", fixed_positions_path])

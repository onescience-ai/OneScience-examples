...
from evo2.data.tokenizer import Evo2Tokenizer
from evo2.utils.config import Evo2PreprocessingConfig, Evo2TaxonomyLineage
...
class Evo2Preprocessor:
    ...
    def preprocess_data(
        ...
        with self.preprocessing_context_manager(
            ...
            # 瀵圭敓鐗╁簭鍒楋紙DNA/RNA锛夎繘琛屾牸寮忓寲銆佷慨楗般€佺瓫閫夈€佹彃鍏ュ垎绫绘爣绛俱€佸垏鍒嗘垚瀛愬簭鍒椼€乼okenize锛屽苟鏈€缁堢粍缁囨垚缁撴瀯鍖栫殑璁粌鏁版嵁
            for seq in seqs_to_parse:
                ...
                # Construct taxonomy token (鍒嗙被鏍囩) with random dropout on the lineage categories per sequence.

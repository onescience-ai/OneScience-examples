...
def add_arguments_eval(parser: ap.ArgumentParser):
    ...
    parser.add_argument(
        "--device",
        default="auto",
        help="Execution device: auto, cpu, cuda, gpu, dcu, cuda:N, or dcu:N (default: auto)",
    )
    parser.add_argument(
        "--protein-embeddings",
        required=False,
...
def run_emb_eval(args):
    ...
    from onescience.utils.state.embedding import compute_gene_overlap_cross_pert, get_precision_config
    from onescience.datapipes.state.embedding.loader import create_dataloader
    from ...embedding.inference import Inference
    ...
    inference = Inference(cfg=cfg, protein_embeds=protein_embeds, device=args.device)

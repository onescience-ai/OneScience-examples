...
def run_tx_train(cfg: DictConfig):
    ...
    from onescience.utils.state.callbacks.transition import (
        BatchSpeedMonitorCallback,
        CumulativeFLOPSCallback,
        GradNormCallback,
        ModelFLOPSUtilizationCallback,
    )
    from ...transition.factory import get_lightning_module
    from onescience.utils.state.transition import get_checkpoint_callbacks, get_loggers
    ...
    target_device = resolve_device(cfg["model"].get("device", "auto"))
    ...
    if target_device.type == "cuda":
        print(f"CUDA/DCU memory allocated: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
        print(f"CUDA/DCU memory reserved: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")
    ...
    if checkpoint_path is None and manual_init is not None:
        ...
        if pert_encoder_weight_key in checkpoint_state:
            ...
            if checkpoint_pert_dim != model.pert_dim or cfg["data"]["kwargs"]["embed_key"] == "X_hvg":
                ...
                from onescience.modules.state.transition.utils import build_mlp

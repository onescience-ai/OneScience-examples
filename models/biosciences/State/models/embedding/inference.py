...
from onescience.modules.state.embedding.model import StateEmbeddingModel
from omegaconf import OmegaConf
from onescience.datapipes.state.embedding.loader import create_dataloader
from onescience.utils.state.embedding import get_embedding_cfg, get_embeddings, get_precision_config
from onescience.utils.state.device import resolve_device

log = logging.getLogger(__name__)


class Inference:
    def __init__(self, cfg=None, protein_embeds=None, device=None):
        self.model = None
        self.collator = None
    ...
    def load_model(self, checkpoint):
        ...
        import onescience.modules.state.embedding as legacy_nn
        import onescience.modules.state.embedding.model as legacy_model
        import onescience.modules.state.embedding.loss as legacy_loss
        import onescience.modules.state.embedding.flash_transformer as legacy_flash
        import onescience.datapipes.state.embedding as legacy_data
        import onescience.datapipes.state.embedding.loader as legacy_loader
        from . import trainer as legacy_trainer
        import onescience.utils.state.embedding as legacy_utils
        import onescience.utils.state.callbacks.embedding as legacy_callbacks

        legacy_root = sys.modules[__package__]

        legacy_train = sys.modules.get("vci.train")
        if legacy_train is None:
            legacy_train = types.ModuleType("vci.train")
            legacy_train.__path__ = []

        sys.modules["vci"] = legacy_root
        sys.modules["vci.nn"] = legacy_nn
        sys.modules["vci.nn.model"] = legacy_model
        sys.modules["vci.nn.loss"] = legacy_loss
        sys.modules["vci.nn.flash_transformer"] = legacy_flash
        sys.modules["vci.data"] = legacy_data
        sys.modules["vci.data.loader"] = legacy_loader
        sys.modules["vci.train"] = legacy_train
        sys.modules["vci.train.trainer"] = legacy_trainer
        sys.modules["vci.train.callbacks"] = legacy_callbacks
        sys.modules["vci.utils"] = legacy_utils

        legacy_root.nn = legacy_nn
        legacy_root.data = legacy_data
        legacy_root.train = legacy_train
        legacy_root.utils = legacy_utils
        legacy_nn.model = legacy_model
        legacy_data.loader = legacy_loader
        legacy_train.trainer = legacy_trainer
        legacy_train.callbacks = legacy_callbacks
        ...
        target_device = resolve_device(self._requested_device)
        device_type = target_device.type
        precision = get_precision_config(device_type=device_type)
        self.model = self.model.to(device=target_device, dtype=precision)
        ...
        if self.protein_embeds is None:
            ...
            try:
                ...
                pass
        all_pe = self.protein_embeds or get_embeddings(self._vci_conf, device=target_device)
        if isinstance(all_pe, dict):
            all_pe = torch.vstack(list(all_pe.values()))
        self.model.pe_embedding = nn.Embedding.from_pretrained(all_pe).to(target_device, dtype=precision)
    ...
    def encode_adata(
        ...
        if lancedb_path is not None:
            from onescience.utils.state.vectordb import StateVectorDB

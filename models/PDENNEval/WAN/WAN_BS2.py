...
class BlackScholesEquation(object):
    ...
    def end_boundary(self, dataset, n=100):
...
def loss(eq, model_u, model_v, xt_int, xt_init, xt_end, ue_end, beta):
    # domain squares size
    sp_s = 1
    te_s = 1
    st_s = sp_s * te_s
    ...
    v_end = model_v(xt_end)
    int_l1 = sp_s * torch.mean(ue_end * (w_end * v_end))
    # u(x,0) * \phi(x,0)
    w_init, _ = fun_w(xt_init[:, :-1], eq)
    u_init, v_init = model_u(xt_init), model_v(xt_init)
    int_r1 = sp_s * torch.mean(u_init * (w_init * v_init))
    ...
    int_r5 = - 0.08 * st_s * torch.mean(torch.sum((dux * (xy_int ** 2)) * (wx * v + w * dvx), dim=1))
...
def train_pipeline():
    ...
    # define equation
    Eq = BlackScholesEquation(DIMENSION, DEVICE)
    # define model
    ...
    optv = torch.optim.Adam(model_v.parameters(), lr=0.0014)

    xt_int = Eq.interior(NUM_INT_SAMPLE)
    xt_init = Eq.init_boundary(n=500)
    xt_end, ue_end = Eq.end_boundary(dataset, n=100)
    ...
    for step in tqdm(range(NUM_ITERATION + 1)):
        ...
        start_t = time.time()
        for _ in range(2):
            loss_u, loss_v = loss(Eq, model_u, model_v, xt_int, xt_init, xt_end, ue_end, BETA)
            optv.zero_grad()
            loss_v.backward()
            optv.step()
        for _ in range(1):

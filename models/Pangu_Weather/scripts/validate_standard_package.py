...
MODEL_ID = "OneScience/Pangu-Weather/"
...
def validate_manifest(path):
    ...
    if re.findall(r"OneScience/Pangu-Weather(?=[\"'\\s])", yaml_text):
        fail("Manifest 中出现不带尾部斜杠的模型 ID")
    ...
    if not command_refs:
        fail("Manifest 未声明任何 command_ref")
...
def main():
    parser = argparse.ArgumentParser(description="校验 Pangu-Weather ModelScope 标准模型包")

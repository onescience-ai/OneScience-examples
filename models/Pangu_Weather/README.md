...
<p align="center">
  <strong>
    <span style="font-size: 20px;">点击下方图片，体验一键式 Pangu-Weather 模型开发</span>
  </strong>
</p>

<p align="center">
  <a href="https://modelscope.cn/models/OneScience/Pangu-Weather/" target="_blank" rel="noopener noreferrer">
...
Pangu-Weather 是面向全球中期天气预报的三维 Earth-Specific Transformer 模型，通常接收 ERA5 再分析数据中的多变量全球格点场，输出未来时刻的地面变量和高空变量预测场。该模型属于 OneScience 的 earth 领域，适合用于天气预报训练、推理、评测和结果可视化。

本仓库整理为 OneScience 标准模型运行包，包含 Pangu-Weather 的训练、推理、评测可视化脚本、模型配置、预检脚本和机器可读 Manifest。模型运行依赖数据集 `OneScience/ERA5`；本次按任务要求只整理模型仓库，不复制、不重打包、不生成 ERA5 数据集标准仓库。
...
|---|---|
| 资源类型 | 模型 |
| ModelScope 模型 ID | `OneScience/Pangu-Weather/` |
| OneScience 领域 | earth |
| 领域标签 | weather_forecast, era5, transformer, global_forecasting |
| 任务 | 全球天气预报 |
| 任务标签 | train, inference, evaluate, visualize |
| 主平台资源 | https://modelscope.cn/models/OneScience/Pangu-Weather/ |
| 标准运行包工作目录 | `.` |
...
| 能力 | 必须提供 |
|---|---|
| `inference` | `python inference.py`；需要 `data/checkpoints/model_bak.pth`、`OneScience/ERA5` 测试年份数据和静态文件 |
| `train` | `python train.py`；需要 `OneScience/ERA5` 训练/验证年份数据和静态文件，输出 checkpoint 到 `data/checkpoints/model_bak.pth` |
| `finetune` | 当前标准包未声明微调能力 |
| `evaluate` | `python result.py`；需要推理输出、训练损失文件和测试年份数据 |
...
| `conf/config.yaml` | 配置文件 | 控制 Pangu-Weather 模型结构、训练轮数、数据路径、年份、变量通道、静态文件、分布式参数和 checkpoint 目录 | 是 | 预检、训练、推理、评测 | `session_workdir/conf/config.yaml` | 已适配为最小验证配置 |
| `scripts/preflight_model.py` | 预检脚本 | 检查配置、数据路径、必需数据文件、变量/schema、静态文件和 checkpoint 状态 | 是 | 预检 | `session_workdir/scripts/preflight_model.py` | 不复制数据；加 `--check-data` 时读取已下载 ERA5 |
...
| `train.py` | 运行脚本 | 训练 Pangu-Weather，并保存 `model_bak.pth`、训练损失和验证损失 | 是 | 训练 | `session_workdir/train.py` | 单卡命令为 `python train.py` |
| `inference.py` | 运行脚本 | 加载 `model_bak.pth`，对测试年份 ERA5 数据执行推理 | 是 | 推理 | `session_workdir/inference.py` | 输出到 `result/output/` |
| `result.py` | 运行脚本 | 计算 RMSE/ACC，并绘制预测、真值、误差图和损失曲线 | 是 | 评测、可视化 | `session_workdir/result.py` | 默认绘制测试年份样例 |
...
| `data/` | 外部数据目录 | 放置从 `OneScience/ERA5` 下载或挂载的 ERA5 HDF5、静态文件和 checkpoint 输出 | 是 | 训练、推理、评测 | `session_workdir/data/` | 本仓库不上传数据文件 |
...
Pangu-Weather 模型依赖 ERA5 HDF5 数据集，数据集 ID 固定为 `OneScience/ERA5`。模型侧 Manifest 在 `relations.required_datasets` 中声明该数据集，并在 `run_matrix` 中说明配置预检、数据预检、训练、推理、评测和可视化分别需要哪些文件与命令。

本次只处理模型标准仓库，不整理数据集文件，也不生成数据集标准仓库。ERA5 数据应从 `OneScience/ERA5` 下载或由 OneScience 环境挂载到 `session_workdir/data/`，目录下至少应包含 `data/<year>.h5` 和 `static/`；HDF5 文件需包含 `fields`、`global_means`、`global_stds`，且 `fields.attrs` 包含 `variables` 和 `time_step`。静态目录需包含 `land_mask.npy`、`soil_type.npy`、`topography.npy`、`geopotential.nc` 和 `land_sea_mask.nc`。
...
```bash
modelscope download --model OneScience/Pangu-Weather/ --local_dir ./earth_pangu_weather
cd ./earth_pangu_weather
```
...
| `Pangu-Weather 静态文件目录不存在` | 缺少 `data/static/` | 从 `OneScience/ERA5` 下载静态文件，或运行 `python fake_data.py` 做本地 smoke test |
| `HDF5 variables 缺少配置通道` | HDF5 `fields.attrs.variables` 与配置通道不匹配 | 使用与 Pangu-Weather 配置匹配的 ERA5 数据，或同步修改 `conf/config.yaml` 和 Manifest 的配置适配说明 |
| `model_bak.pth` 不存在 | 尚未训练或未提供 checkpoint | 先执行 `python train.py`，或把已有 checkpoint 放到 `data/checkpoints/model_bak.pth` |
...
| 输出路径 | 类型 | 产生阶段 | 说明 |
|---|---|---|---|
| `data/checkpoints/model_bak.pth` | checkpoint | 训练 | Pangu-Weather 模型权重和优化器状态 |
...
本标准包默认配置面向最小验证：`max_epoch` 为 1，训练、验证、测试年份分别为 1951、1952、1953，分布式开关关闭，并显式设置 `datapipe.dataset.static_dir` 为 `./data/static`。正式训练或 benchmark 前，应根据 `OneScience/ERA5` 实际可用年份、硬件规模和实验目标调整配置，并同步更新 Manifest 中的 `configuration_adaptation`。
